...
from cryton.lib.util import creator, exceptions


@pytest.mark.django_db
class TestCreator:
    def test_create_plan(self, mocker):
        mock_plan = mocker.patch("cryton.lib.util.creator.plan.Plan")
        mock_stage = mocker.patch("cryton.lib.util.creator.stage.Stage")
        mocker.patch("cryton.lib.util.creator.stage.StageModel")
        mock_create_stage = mocker.patch("cryton.lib.util.creator.create_stage")
    ...
    def test_create_plan_dependency_error(self, mocker):
        mocker.patch("cryton.lib.util.creator.create_stage")
        mocker.patch("cryton.lib.util.creator.stage.Stage")
        mock_plan = mocker.patch("cryton.lib.util.creator.plan.Plan")
        mock_stage_model_get = mocker.patch("cryton.lib.util.creator.stage.StageModel.objects.get")
    ...
    def test_create_stage(self, mocker):
        mock_stage = mocker.patch("cryton.lib.util.creator.stage.Stage")
        mock_create_step = mocker.patch("cryton.lib.util.creator.create_step")
        mocker.patch("cryton.lib.util.creator.step.Step")
        mock_create_successor = mocker.patch("cryton.lib.util.creator.create_successor")
    ...
    def test_create_step(self, mocker, test_step_dict):
        mock_step = mocker.patch("cryton.lib.util.creator.step.Step")
        mock_create_mapping = mocker.patch("cryton.lib.util.creator.create_output_mapping")
    ...
    def test_create_successor(self, mocker, test_values):
        mocker.patch("cryton.lib.util.creator.step.StepModel.objects.get")
    ...
    def test_create_successor_non_existent(self, mocker):
        mock_step_model_get = mocker.patch("cryton.lib.util.creator.step.StepModel.objects.get")
    ...
    def test_create_output_mapping(self, mocker):
        mock_mapping_create = mocker.patch("cryton.lib.util.creator.OutputMapping.objects.create")

        creator.create_output_mapping({"name_from": "", "name_to": ""}, 0)

        mock_mapping_create.assert_called()

    @pytest.mark.parametrize("w_name, w_address, w_prefix", [("test", "test", ""), ("test", "test", "test")])
    def test_create_worker(self, mocker, w_name, w_address, w_prefix):
        mock_worker = mocker.patch("cryton.lib.util.creator.worker.Worker")
        mock_worker.return_value.model.id = 0

        result = creator.create_worker(w_name, w_address, w_prefix)

        assert result == 0

    @pytest.mark.parametrize("w_name, w_address, w_prefix", [("", "", ""), ("test", "", "")])
    def test_create_worker_error(self, w_name, w_address, w_prefix):
        with pytest.raises(exceptions.WrongParameterError):
            creator.create_worker(w_name, w_address, w_prefix)
